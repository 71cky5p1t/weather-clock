...
- `QUOTE` – rotates through `defaults.quotes`; the words fade in one by one, then the quote holds for the rest of `durationMs`. `CONTENT` – any PNG/JPG/GIF/WebP dropped into `tsv-radar/content/` (name = filename without extension).
